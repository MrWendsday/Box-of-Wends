...
SELECT * from customers c WHERE c.state = 'NY' or c.state = 'NJ';

UPDATE customers set contactLastName = 'haseeb' where customerNumber = 131;

SELECT * from customers c WHERE c.state = 'NY' or c.state = 'NJ';

SELECT * from customers WHERE customerName LIKE 'a_%_%';

SELECT count(c.city), c.state from customers c group by c.state;

SELECT c.customerName, e.employeeNumber, e.firstName, off.officeCode, off.city FROM employees e
